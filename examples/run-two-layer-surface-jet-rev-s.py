...
U2 = -0.005 * np.ones((nx))
m = pyqg.QGModel(tavestart=0, dt=500, U1=U1,U2=U2, nx=nx, tmax=500*500*1000, ntd=4, beta=-1.5e-11)
print(m.beta)
fk = m.wv != 0
...
for snapshot in m.run_with_snapshots(
    ...
    plt.pause(0.01)
    plt.draw()
    plt.savefig('Q_full_with_surf_jet_rev_s_' + ('%04d' % i) + '.png', bbox_inches='tight')
    i += 1
subprocess.call("ffmpeg -y -f image2 -i Q_full_with_surf_jet_rev_s_%04d.png -f mp4 -vcodec libx264 -r 10 -pix_fmt yuv420p -vf scale=978:844 q_full_with_surf_jet_rev_s.mp4",shell=True)
subprocess.call("ffmpeg -y -f image2 -i Q_with_surf_jet_rev_s_%04d.png -f mp4 -vcodec libx264 -r 10 -pix_fmt yuv420p -vf scale=978:844 q_with_surf_jet_rev_s.mp4",shell=True)
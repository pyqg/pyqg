...
Q1 = np.expand_dims(m.beta + m.F1*(m.U1 - m.U2), axis=1) * m.y + np.expand_dims(np.gradient(m.U1, m.dy),axis=1)
# + np.gradient(m.U1, m.dy)
# plt.imshow(Q1)
# plt.show()
...
for snapshot in m.run_with_snapshots(
    ...
    plt.pause(0.01)
    plt.draw()
    plt.savefig('Q_full_with_surf_jet_s_' + ('%04d' % i) + '.png', bbox_inches='tight')
    ...
    plt.pause(0.01)
    plt.draw()
    plt.savefig('Q_with_surf_jet_s_' + ('%04d' % i) + '.png', bbox_inches='tight')
    i += 1
subprocess.call("ffmpeg -y -f image2 -i Q_full_with_surf_jet_s_%04d.png -f mp4 -vcodec libx264 -r 10 -pix_fmt yuv420p -vf scale=978:844 q_full_with_surf_jet_s.mp4",shell=True)
subprocess.call("ffmpeg -y -f image2 -i Q_with_surf_jet_s_%04d.png -f mp4 -vcodec libx264 -r 10 -pix_fmt yuv420p -vf scale=978:844 q_with_surf_jet_s.mp4",shell=True)
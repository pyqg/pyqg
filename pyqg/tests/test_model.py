...
class PyqgModelTester(unittest.TestCase):
    ...
    def test_subgrid_forcing(self):
        ...
        Sq3 = m.advection_subgrid_forcing(32, 'q',
        ...
        Sq4 = m.advection_subgrid_forcing(32, 'q',
        ...
        res_vals = [62, 38, 6]
        forcings = [m.advection_subgrid_forcing(res) for res in res_vals]
        ...
        Su, Sv, Sq = m.advection_subgrid_forcing(16, ['u','v','q'])
        assert Su.shape == (2,16,16)
        assert Sv.shape == (2,16,16)
        assert Sq.shape == (2,16,16)

        # test for invalid arguments
        with pytest.raises(AssertionError): m.advection_subgrid_forcing(64)
        with pytest.raises(AssertionError): m.advection_subgrid_forcing(63)
        with pytest.raises(AssertionError): m.advection_subgrid_forcing(8, 'qh')
        with pytest.raises(AssertionError): m.advection_subgrid_forcing(8, ':)')
